...
def alarm():
    os.system('spd-say "wally I got you"')
    time.sleep(3)
...
if __name__ == '__main__':
    ...
    # [2] Deploy YOLOv5 Detection Model
    ...
    # [3] Initialize Drone Settings
    drone = Drone(
        # stable parameters
        #kp = np.array([0.115, 0.115, 0.1, 25]),
        kp = np.array([0.115, 0.345, 0.3, 25]),
        #ki = np.array([0.001, 0.001, 0, 0]),
        ki = np.array([0.002, 0.002, 0, 0]),
        #kd = np.array([0.12, 0.12, 0, 0])
        kd = np.array([0.48, 0.48, 0.1, 0])
    )
        ...
        if (fr % r == 0):
            ...
            if target in mydick:
                ...
                if is_aimmed or 960*2/5 < center_x < 960*3/5 and 720*2/5 < center_y < 720*3/5:
                    ...
                    drone.send_rc_control(*map(int, [0, result[2], result[1], result[0]]))
